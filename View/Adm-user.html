<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/png" href="Icones/20200719_214002.jpg" />
    <link rel="stylesheet" href="../assets/CSS/aos.css">
    <link rel="stylesheet" href="../assets/CSS/admsystem.css">
    <link rel="stylesheet" href="../assets/CSS/dropzone.css">
    <link rel="stylesheet" href="../assets/CSS/contact.css">
    <link rel="stylesheet" href="../assets/CSS/admlogin.css">
    <script src="https://kit.fontawesome.com/77e2f3b2cf.js" crossorigin="anonymous"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.carousel.css" />

    <title>BlogPam | Gestion Des Utilisateurs</title>
</head>

<body>


    <div class="adm-desktop">
        <div class="header-menu">
            <div class="hm1">
                <h4>BlogPam <span class="hk">Administration</span></h4>
            </div>
            <div class="hm2">
                <img src="../assets/Icones/icons8_Male_User_48px.png" alt=""> <span>Utilisateur Nom</span>
            </div>
        </div>
        <div class="adm-desk">
            <div class="menu-adm">
                <div class="nav-adm">
                    <ul>
                        <li><a href="Adm-system.html">Statistique</a></li>
                        <li><a href="Adm-article.html">Article</a></li>
                        <li><a id="wh" href="Adm-user.html">Utilisateur</a></li>
                    </ul>
                </div>
            </div>
            <div class="menu-block" id="style-3">
                <div class="container">
                    <ul class="navigation">
                        <li><a id="tab-nav-1" class="nav" data-tab="tab-1" href="#">Liste Des Utilisateurs</a></li>
                        <li><a id="tab-nav-2" class="nav default" data-tab="tab-2" href="#">Ajouter Utilisateur</a></li>
                    </ul>

                    <div class="wrapper-tabs">
                        <div id="tab-1" class="tab-item ">
                            <div class="article-adm">
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                            </div>
                            <div class="article-adm">
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                            </div>

                            <div class="article-adm">
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                                <div class="divi">
                                    <h4>Nom Prenom</h4>
                                    <a href=""><img src="../assets/ICONES/icons8_Edit_File_24px.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Delete_24px_1.png" alt=""></a>
                                    <a href=""><img src="../assets/ICONES/icons8_Error_24px_2.png" alt=""></a>
                                </div>
                            </div>
                        </div>
                        <div id="tab-2" class="tab-item hide">
                            <div style="width: 70%;">
                                <div class="form-log">
                                
                                    <form action="">
                                        <div class="item-log">
                                            <input type="text" placeholder="Nom">
                                        </div>
                                        <div class="item-log">
                                            <input type="text" placeholder="Prenom">
                                        </div>

                                        <div class="item-log">
                                            <input type="email" placeholder="Email">
                                        </div>
                                        <div class="item-log">
                                            <input type="text" placeholder="Telephone">
                                        </div>
                                        <div class="item-log">
                                            <input type="password" placeholder="Mot de passe">
                                        </div>
                                        <div class="item-log">
                                            <input type="password" placeholder="Confirmer Mot de passe">
                                        </div>
                                      
    
                                        <div class="item-log-r">
                                            <label for=""><b>Type User</b></label><br>
                                            <input type="radio" id="male" name="gender" value="male">
                                            <label for="male">Administrateur</label><br>
                                            <input type="radio" id="female" name="gender" value="female">
                                            <label for="female">Invite</label><br>
    
                                        </div>
                                        <br>
                                        <div class="item-log">
                                            <button type="submit" class="btn-sub4">Ajouter</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>





    <div class="none">
        <div class="adm-mobile">
            <div class="aff-error">
                <h1>Error</h1>
                <p class="fg">
                    Espace Administration disponible seulement sur pc
                </p>
                <p>
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Excepturi, sit! Temporibus corrupti quidem
                    assumenda atque laboriosam. Perferendis debitis inventore harum ad eius sunt quasi, esse quia atque,
                    magni consequatur tempore?
                </p>

                <div>
                    <a href="">Page Accueil</a>
                </div>
            </div>
        </div>
    </div>

    <script src="../assets/JS/jquery.min.js"></script>
    <script src="../assets/JS/tab.js"></script>
    <script src="../assets/JS/aos.js"></script>
    <script src="../assets/JS/Dropzone.js"></script>
    <script src="../assets/JS/editor.js"></script>
    <script>
        AOS.init();
        $(document).ready(function () {
            scroller.init();
            $(".owl-carousel").owlCarousel({
                margin: 5,
                loop: true,
                autoWidth: true,
                items: 2
            });
        });
    </script>
</body>

</html>